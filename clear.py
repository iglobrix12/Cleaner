...
def search_and_replace_in_process(process_name, search_strings, replace_string=""):
    ...
    if pid is None:
        print(f"Процесс не найден '{process_name}'")
        return

    process_handle = OpenProcess(PROCESS_ALL_ACCESS, False, pid)
    if not process_handle:
        print(f"не найден процесс '{process_name}'")
        return
    ...
    while VirtualQueryEx(process_handle, address, ctypes.byref(memory_info), ctypes.sizeof(memory_info)):
        ...
        if ReadProcessMemory(process_handle, base_address, buffer, region_size, ctypes.byref(bytes_read)):
            ...
            for search_string in search_strings:
                ...
                offset = buffer_string.find(search_bytes)
                if offset != -1:
                    print(f"найдено '{search_string}'")
                    new_buffer = (buffer_string[:offset] + replace_string.encode() + buffer_string[offset+len(search_bytes):])
...
if __name__ == "__main__":
    search_strings = ["Что очистить"]
    search_and_replace_in_process("Проццес.exe", search_strings, replace_string="")
